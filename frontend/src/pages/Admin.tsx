...
import { Mail, Upload, Music, Image, MessageSquare, Trash2, RefreshCw } from 'lucide-react';
...
const Admin = () => {
  ...
  const ADMIN_PASSWORD = '2026rockey@12';
  const AUTH_HEADER = { 'Authorization': 'Bearer visualix-admin-secret-2026' };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (activeTab === 'Portfolio') {
        await axios.post('/api/v1/portfolio', {
  ...
  return (
    <div className="min-h-screen py-16 px-6 max-w-6xl mx-auto">
      ...
      <AnimatedSection direction="down" className="flex justify-between items-center mb-10">
        ...
        <button onClick={() => setIsAuthenticated(false)} className="px-4 py-2 border border-royal-gold/30 text-royal-gold rounded-lg hover:bg-royal-gold/10 transition text-sm">
          Logout
        </button>
      </AnimatedSection>
        ...
        <AnimatedSection direction="up" delay={0.15}>
          <div className="glass-panel p-8 rounded-2xl">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-2xl font-serif text-white">Upload to {activeTab}</h3>
            </div>
